"""MLP evaluation workflow for configured eval runs or all dataset runs."""
...
__all__ = ["EvaluationScope", "evaluate_model", "load_bounds_json"]


def load_bounds_json(path: Path) -> dict[str, ColumnBounds]:
    """Load scaler bounds JSON written by the MLP training workflow."""
...
def evaluate_model(
    ...
    """Run MLP inference and write metrics for the requested scope."""
    ...
    for run_key in run_keys:
        ...
        y_hat_scaled = model.predict(x_2d, batch_size=batch_size, verbose=0).reshape(-1)
