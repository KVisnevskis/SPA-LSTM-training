...
def test_load_experiment_config() -> None:
    ...
    assert cfg.model.hidden_units is None
...
def test_load_experiment_config_parses_hidden_units(tmp_path: Path) -> None:
    config_path = tmp_path / "custom_width.yaml"
    config_path.write_text(
        "\n".join(
            [
                "name: custom_width",
    ...
    assert cfg.model.hidden_units == 128
