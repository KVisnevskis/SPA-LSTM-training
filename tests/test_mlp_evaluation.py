...
def _make_cfg(tmp_path, *, run_name: str, eval_runs: list[str] | None = None) -> ExperimentConfig:  # noqa: ANN001
    ...
    return ExperimentConfig(
        ...
        training=TrainingConfig(epochs=5, patience=1, batch_size=8, seed=42, verbose=0),
        runtime=RuntimeConfig(output_dir=str(tmp_path / "outputs"), run_name=run_name),
    )


def test_evaluate_model_uses_prescaled_bounds_and_rank2_shape(tmp_path, monkeypatch) -> None:
    cfg = _make_cfg(tmp_path, run_name="mlp_eval_smoke", eval_runs=["eval_a"])
    ...
    monkeypatch.setattr(
        "mlp.evaluation.load_runs_as_dataframes",
        lambda _h5, run_keys, _required: {run_key: eval_df.copy() for run_key in run_keys},
    )
    monkeypatch.setattr(
        "mlp.evaluation.load_hdf5_scaler_bounds",
        lambda _h5, columns: {
    ...
    out_dir = tmp_path / "outputs" / "mlp_eval_smoke"
    ...
    assert fake_model.predict_calls == [((3, 4), 8, 0)]
...
def test_evaluate_model_all_scope_tracks_split_roles(tmp_path, monkeypatch) -> None:
    cfg = _make_cfg(tmp_path, run_name="mlp_eval_all_smoke", eval_runs=["eval_a"])
    ...
    out_dir = tmp_path / "outputs" / "mlp_eval_all_smoke"
