#!/usr/bin/env python3
"""Plot stacked baseline time-series comparisons by layer depth on eval runs.
...
MODEL_GROUPS: dict[str, tuple[str, ...]] = {
    "univariate_depth": ("SLU-LSTM", "TLU-LSTM"),
    "multivariate_depth": ("SLM-LSTM", "TLM-LSTM"),
}
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/figures/ch4/baseline_eval_timeseries_stacked_layer_depth.pdf"),
        help=(
    ...
    parser.add_argument(
        "--groups",
        default="univariate_depth,multivariate_depth",
        help=(
            "Comma-separated model groups to plot. Allowed values: "
            "univariate_depth, multivariate_depth "
            "(default: univariate_depth,multivariate_depth)."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if not requested_groups:
        raise ValueError("No groups selected. Use --groups univariate_depth,multivariate_depth")
